...
/**
 * Activity Log Page - Client Component
 */
      ...
      try {
        ...
        while (!user && attempts < maxAttempts) {
          ...
          if (!user && attempts < maxAttempts) {
            console.log(`📊 Activity: No user found, retrying... (attempt ${attempts}/${maxAttempts})`)
            await new Promise(resolve => setTimeout(resolve, 300))
          }
        }

        if (!user) {
          console.error('📊 Activity: No user found after retries')
          setLoading(false)
          return
        }

        console.log('📊 Activity: Loading data for', user.email)
        ...
        const successfulCommands = successfulCommandsResult.count || 0
        const successRate = totalCommands ? Math.round((successfulCommands / totalCommands) * 100) : 0
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <StatCard
            ...
            valueColor={stats.successRate >= 95 ? 'text-green-600' : stats.successRate >= 80 ? 'text-yellow-600' : 'text-red-600'}
          />
        </div>
