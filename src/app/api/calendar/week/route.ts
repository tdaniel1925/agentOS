 ...
 * Calendar Week API
 *
 * Get this week's calendar events for the subscriber
 */
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    console.log(`📅 Calendar week requested for subscriber: ${subscriber_id}`)
    ...
    const events = await getWeekCalendarEvents(accessToken)
    ...
    await (supabase as any)
      ...
      .insert({
        ...
        success: true,
        response_sent: `Found ${events.length} event${events.length === 1 ? '' : 's'} this week`,
        metadata: {
